...
class DriveBase_Controller(object):
    def init(self, lfmotor: Motor, rgmotor: Motor, syslfmotor: Motor, sysrgmotor: Motor, oneunit: int, scaleunit: int, wheelerror: int = None, distbetweenwheel: int = 0, wheelcircumference: int = 0) -> None:
    ...
    async def initrun(self) -> None:
        ...
        self.PID.init() # ---> Init full PID, but __initvalues() might also work <--- #
    ...
    async def gyroforwards(self, distance: int, speed1: int, typestop, offsetlf: int = 0, offsetrg: int = 0) -> None:
        ...
        if(self.lasttfunction == 2): self.PID.initvalues()
    ...
    async def gyrobackwards(self, distance: int, speed1: int, typestop, offsetlf: int = 0, offsetrg: int = 0) -> None:
        ...
        # ---> Reset PID Values after some time <--- #
        if(self.lasttfunction == 1): self.PID.initvalues()
...
class runmethods(object):
    ...
    async def runk2(self): 
        ...
        #-->mission 4+3. 30+30 <--# 
        await mydrivebase.gyrobackwards(700, 1100, typestop=mydrivebase.braketank)
        await mydrivebase.turnleftt(90, 90, lfsign=0, rgsign=1, typestop=mydrivebase.braketank)
        await mydrivebase.gyroforwards(100, 100, typestop=mydrivebase.braketank)
        await rundegrees(mydrivebase.sysrighttmotor, 300, 300)
        await rundegrees(mydrivebase.syslefttmotor, 20, 30)
        


        #--> return <--#

        await mydrivebase.gyrobackwards(100, 300, typestop=mydrivebase.braketank)
        await mydrivebase.turnleftt(90, 140, lfsign=-1, rgsign=0, typestop=mydrivebase.braketank)
        await mydrivebase.gyrobackwards(700, 1100, typestop=mydrivebase.braketank)
    ...
    async def runk3(self):
        ...
        #--> mission 1. 10+10p <--# 
        await mydrivebase.gyroforwards(630, 700, typestop = mydrivebase.braketank);await wait(200)
        ...
        await multitask(
            ...
            rundegrees(mydrivebase.syslefttmotor, 200, 300)
        ); await wait(200)
        ...
        await mydrivebase.turnleftt(95, 140, typestop=mydrivebase.braketank)
        await rundegrees(nydrivebase.sysrighttmotor, 170, 400)


        #--> mission 2. 10+10+10p <--#
        await mydrivebase.gyrofarwards(300, 600, typestop=mydrivebase.braketank)
        ...
        await mydrivebase.turnrightt(50, 100, lfsign=1, rgsign=-1, typestop=mydrivebase.braketank)
        await mydrivebase.gyrobackwards(700, 1100, typestop=mydrivebase.braketank)
