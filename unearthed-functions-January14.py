...
class runmethods(object):
    ...
    async def runk1(self): 
        ...
        await mydrivebase.turnleftt(90, 90, typestop=mydrivebase.braketank); await wait(500)
        await mydrivebase.gyroforwards(170, 200, typestop=mydrivebase.braketank)

        await multitask(
            rundegrees(mydrivebase.syslefttmotor, -70, 200),
            rundegrees(mydrivebase.sysrighttmotor, 900, 600)
        );

        await mydrivebase.gyrobackwards(140, 300, typestop=mydrivebase.braketank);await wait(200)
        await mydrivebase.turnrightt(100, 130, typestop=mydrivebase.braketank);await wait(200)
    ...
    async def runk7(self):
        ...
        mydrivebase.PID.setconstants(1175, 750, 2, 0, 1000); await wait(200); # ~small deadzone, because we need to be really precise with all movements (tested)
        ...
        await mydrivebase.gyroforwards(440, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)
        await mydrivebase.turnleftt(27, 32, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(180, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        print("reached phase 2"); mydrivebase.hub.light.on(Color.BLACK)
        await mydrivebase.turnrightt(70, 32, lfsign = 1, rgsign = 0, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(12, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        await runanglestalled(mydrivebase.sysrighttmotor, 1100, -1400); await wait(250) # ---> Get minecart (in phase 2) <--- #

        await mydrivebase.gyrobackwards(75, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        await mydrivebase.turnleftt(98, 32, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(485, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        # # ---> Align to mission <--- #
        await mydrivebase.turnleftt(83, 32, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyrobackwards(150, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)
