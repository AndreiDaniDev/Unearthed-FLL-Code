...
class DriveBase_Controller(object):
    ...
    async def gyroforwards(self, distance: int, speed1: int, typestop, offsetlf: int = 0, offsetrg: int = 0, initpid: int = 0) -> None:
        ...
        # ---> Need to implement time and speeding functions (simple implementation, with no strain on the processor) <--- #
    ...
    async def gyrobackwards(self, distance: int, speed1: int, typestop, offsetlf: int = 0, offsetrg: int = 0, initpid: int = 0) -> None:
...
class runmethods(object):
    ...
    async def runk4(self) -> None: 
        ...
        await mydrivebase.gyroforwards(815, 925, typestop = mydrivebase.braketank); await wait(500)

        await mydrivebase.turnrightt(93, 50, lfsign = 1, rgsign = 0, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(180, 775, typestop = mydrivebase.braketank); # await wait(250)  

        await mydrivebase.sysrighttmotor.run_angle(1000, -1250); 

        await mydrivebase.gyrobackwards(315, 750, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(100, 1100, typestop = mydrivebase.braketank); await wait(250)

        await mydrivebase.turnleftt(60, 60, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(750, 1100, typestop = mydrivebase.braketank); await wait(250)
        
        # ------------------------------------------------------------------------------------------ #

        return None

    async def runk5(self) -> None: # ---> Stones (needs a lot of tuning with the new sistem) <--- #
    ...
    async def runk6(self) -> None: # ---> ahh mision (with the gears) <--- # (swapped with runkx with stones (next one)) 
    ...
    # async def runk8(self) -> None: return None
    # async def runk9(self) -> None: return None

    async def runk11(self) -> None:
