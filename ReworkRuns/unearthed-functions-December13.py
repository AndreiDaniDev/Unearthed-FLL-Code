...
class runmethods(object):
    ...
    async def runk7(self):
        ...
        mydrivebase.sysrighttmotor.run_angle(675, 90); # ---> Lift rightt (minecart) arm <---  
        ...
        await mydrivebase.gyroforwards(380, 750, typestop = mydrivebase.braketank); await wait(250); # >>> old dist = 400 <<< #

        await mydrivebase.turnleftt(92, 34, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(24, 750, typestop = mydrivebase.braketank); await wait(250)

        mydrivebase.sysrighttmotor.run_angle(1100, 185); # ---> Lower rightt (minecart) arm <---  
        await mydrivebase.syslefttmotor.run_angle(1100, -3250); await wait(250) # ---> Lift leftt arm <---        
        
        mydrivebase.sysrighttmotor.run_angle(1100, -85); # ---> Lift rightt (without minecart) arm <---  

        await mydrivebase.turnleftt(15, 42, typestop = mydrivebase.braketank); await wait(250) # >>> May work without this <<< #
        await mydrivebase.turnrightt(15, 42, typestop = mydrivebase.braketank); await wait(250) # >>> May work without this <<< #
        
        mydrivebase.syslefttmotor.run_angle(1100, -625); # ---> Relift (for the x-th time) <--- #
        await mydrivebase.gyroforwards(32, 1100, typestop = mydrivebase.braketank); await wait(250) # >>> May work without this <<< #
