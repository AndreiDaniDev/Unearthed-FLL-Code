...
class runmethods(object):
    ...
    async def runk7(self):
        ...
        mydrivebase.PID.setconstants(2150, 2700, 4, deadzone = 0, alphanoise = 1000); await wait(200); # ~small deadzone, because we need to be really precise with all movements (tested)
        
        # ------------------------------------------------------------------------------------------ #
        
        await mydrivebase.trytimelimit(lambda: mydrivebase.sysrighttmotor.run_angle(1100, -590), 250, lambda: mydrivebase.sysrighttmotor.hold()); # ---> force arm <--- #
