...
class DriveBase_Controller(object):
    ...
    async def trytimelimit(self, func, ttime: int, typestop) -> None:
        ...
        if(typestop == None or func == None):
            print("# invalid tle try #"); return None;
...
class runmethods(object):
    ...
    async def runk5(self): # ---> Stones (needs a lot of tuning with the new sistem) <--- #
        ...
        mydrivebase.PID.setconstants(775, 0, 2, deadzone = 1, alphanoise = 750); await wait(200)
    ...
    async def runk7(self):
        ...
        mydrivebase.PID.setconstants(1475, 1750, 3, 0, 1000); await wait(200); # ~small deadzone, because we need to be really precise with all movements (tested)
        ...
        await mydrivebase.gyroforwards(28, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        mydrivebase.syslefttmotor.run_angle(1000, -3500); await wait(1250); # ---> Leave all elements <--- #
        await mydrivebase.trytimelimit(lambda: mydrivebase.turnleftt(32, 32, lfsign = -1, rgsign = 1, typestop = mydrivebase.braketank), 5000, mydrivebase.braketank); await wait(250)
