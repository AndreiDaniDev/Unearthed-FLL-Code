...
class runmethods(object):
    ...
    async def runk1(self): 
        ...
        mydrivebase.PID.setconstants(1925, 1000, 3, alphanoise = 1000, deadzone = 0); await wait(200)
        
        # ------------------------------------------------------------------------------------------ #

        # mydrivebase.syslefttmotor.run_angle(1100, +25); # 250 init, -500 force 
        ...
        await mydrivebase.gyrobackwards(723, 975, typestop = mydrivebase.braketank, initpid = 1); await wait(250)
        
        mydrivebase.sysrighttmotor.run_angle(750, 475); # -50 init, -325 lift, 625 lower
        await mydrivebase.turnleftt(90, 46, lfsign = -1, rgsign = 1, typestop = mydrivebase.braketank, lowerspeedbound = 45); await wait(250)
        ...
        await mydrivebase.sysrighttmotor.run_angle(400, -425); await wait(250) # -50 init, -325 lift, 625 lower     
        ...
        await mydrivebase.gyroforwards(275, 825, typestop = mydrivebase.braketank, initpid = 1); await wait(250)
        
        await multitask(
            mydrivebase.trytimelimit(lambda: mydrivebase.sysrighttmotor.run_angle(275, -300), 3000, typestop = mydrivebase.sysrighttmotor.hold),# ---> if we have a bad run and can't complete the shark we will get tle and exit :D <--- #
            mydrivebase.trytimelimit(lambda: mydrivebase.turnleftt(10, 32, lfsign = -1, rgsign = 1, typestop = mydrivebase.braketank, lowerspeedbound = 32), 2000, mydrivebase.braketank)
        ...
        await mydrivebase.turnrightt(75, 50, lfsign = 1, rgsign = -1, typestop = mydrivebase.braketank); await wait(250)

        await mydrivebase.gyroforwards(750, 1100, typestop = mydrivebase.braketank, initpid = 0); await wait(250)
    ...
    async def runk4(self):
        ...
        mydrivebase.PID.setconstants(1000, 0, 3, alphanoise = 875, deadzone = 0); await wait(200)

        # ------------------------------------------------------------------------------------------ #

        await mydrivebase.gyroforwards(90, 555, typestop = mydrivebase.braketank, initpid = 1); await wait(500)

        await mydrivebase.turnrightt(83, 45, lfsign = 0, rgsign = -1, typestop = mydrivebase.braketank); await wait(250)

        await mydrivebase.gyroforwards(830, 925, typestop = mydrivebase.braketank, initpid = 0); await wait(500)

        await mydrivebase.turnrightt(96, 55, lfsign = 1, rgsign = 0, typestop = mydrivebase.braketank); await wait(250)
        mydrivebase.sysrighttmotor.run(-275); # rotate with a super small speed to allign even if they don't match :)
        await mydrivebase.gyroforwards(210, 1100, typestop = mydrivebase.braketank, initpid = 0); # await wait(500)
        
        await mydrivebase.sysrighttmotor.run_angle(1000, -1250); 
 
        await mydrivebase.gyrobackwards(325, 750, typestop = mydrivebase.braketank, initpid = 0); await wait(250)
