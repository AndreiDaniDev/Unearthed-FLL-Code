...
class PID_Controller(object):
    ...
    def setconstants(self, dkp: int, dkd: int, dki: int) -> None:
        self.__initvalues__(); self.kp = dkp; self.kd = dkd; self.ki = dki; return None
...
class runmethods(object):
    ...
    async def runk3(self):
        ...
        #-->mission 1 <---#
        await mydrivebase.gyroforwards(630, 600, typestop = mydrivebase.braketank)
        await mydrivebase.gyrobackwards(70, 700, typestop = mydrivebase.braketank)

        await rundegrees(mydrivebase.syslefttmotor, 200, 200)
        await mydrivebase.gyroforwards(40, 400, typestop=mydrivebase.braketank)
    
        await rundegrees(mydrivebase.syslefttmotor, -210, 600);await wait(200)
        await rundegrees(mydrivebase.syslefttmotor, 200, 200)
        
        #-->mission 15<---#
        await mydrivebase.turnrightt(46, 90, typestop=mydrivebase.braketank)
        await mydrivebase.gyroforwards(120, 500, typestop=mydrivebase.braketank)

        await mydrivebase.turnleftt(95, 140, rgsign=0, typestop=mydrivebase.braketank);await wait(200)
        await rundegrees(mydrivebase.sysrighttmotor, 170, 300);await wait(200)

        #-->mission 2<---#
        await mydrivebase.gyroforwards(250, 700, typestop=mydrivebase.braketank)
        await rundegrees(mydrivebase.sysrighttmotor, -170, 300)
    ...
    async def runk7(self):
        ...
        # ---> Notes <---
        # sysrighttmotor - 375 = down
        # sysrighttmoror - 250 = perpendicular
        ...
        mydrivebase.sysrighttmotor.run_angle(675, 135); # ---> Lift rightt (minecart) arm <---  

        await mydrivebase.gyroforwards(622, 575, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.turnleftt(60, 24, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(325)

        # ---> We need to turn the leftt arm up and the right down to pe perpendicular with the map <---
        await mydrivebase.gyroforwards(352, 575, typestop = mydrivebase.braketank); await wait(250)
        
        await multitask(
            mydrivebase.sysrighttmotor.run_angle(675, -60),
            mydrivebase.syslefttmotor.run_angle(1100, 425)
        ); await wait(250)
        
        await mydrivebase.gyroforwards(25, 575, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyrobackwards(120, 1100, typestop = mydrivebase.braketank); await wait(250)

        await mydrivebase.turnleftt(34, 32, lfsign = -1, rgsign = 0, typestop = mydrivebase.braketank); await wait(250)
        
        mydrivebase.syslefttmotor.run_angle(1100, -425); # ---> Lift back one more time <--- #
        await mydrivebase.gyroforwards(775, 625, typestop = mydrivebase.braketank); await wait(250)
        
        await mydrivebase.turnleftt(105, 24, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.syslefttmotor.run_angle(1100, 450); # ---> Let down one more time (after turn) <--- #
        
        await mydrivebase.gyroforwards(20, 625, typestop = mydrivebase.braketank); await wait(250)
        
        mydrivebase.sysrighttmotor.run_angle(675, 155); # ---> Lower Minecart <--- #
        mydrivebase.syslefttmotor.run_angle(1100, -1750); # ---> Lift one more time <--- #
        
        await mydrivebase.turnleftt(30, 42, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.turnrightt(30, 42, lfsign = -1, rgsign = 0, typestop = mydrivebase.braketank); await wait(250)
