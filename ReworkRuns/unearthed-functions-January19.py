...
class runmethods(object):
    ...
    async def runk1(self): 
        ...
        await mydrivebase.gyroforwards(160, 180, typestop=mydrivebase.braketank)

        await multitask(
            rundegrees(mydrivebase.syslefttmotor, -70, 200),
            rundegrees(mydrivebase.sysrighttmotor, 900, 500)
        );

        await mydrivebase.gyrobackwards(140, 450, typestop=mydrivebase.braketank);await wait(200)
        await mydrivebase.turnrightt(105, 50, typestop=mydrivebase.braketank);await wait(200)
        await mydrivebase.gyroforwards(650, 1100, typestop=mydrivebase.braketank)
    ...
    async def runk7(self):
        ...
        mydrivebase.PID.setconstants(1375, 1175, 2, 0, 1000); await wait(200); # ~small deadzone, because we need to be really precise with all movements (tested)

        await mydrivebase.gyrobackwards(5, 1000, typestop = mydrivebase.braketank, initpid = 1); await wait(250)
        await mydrivebase.gyroforwards(295, 750, typestop = mydrivebase.holdtank, initpid = 1); await wait(250)
        await mydrivebase.turnleftt(42, 32, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)

        await mydrivebase.gyroforwards(475, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)
        await mydrivebase.turnleftt(48, 32, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(155, 750, typestop = mydrivebase.holdtank, initpid = 0, typeemsk = 3); await wait(250)

        # ---> init phase 2 with minecart <--- #
        await mydrivebase.turnrightt(90, 32, lfsign = 1, rgsign = 0, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(64, mydrivebase.mingoodspeed, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        await runanglestalled(mydrivebase.sysrighttmotor, 1100, 800); await wait(250) # ---> Get minecart (in phase 2) <--- #

        await mydrivebase.gyrobackwards(120, mydrivebase.mingoodspeed, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        await mydrivebase.turnleftt(100, 32, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyroforwards(452, 750, typestop = mydrivebase.holdtank, initpid = 0); await wait(250)

        await mydrivebase.turnleftt(75, 32, lfsign = 0, rgsign = 1, typestop = mydrivebase.braketank); await wait(250)
        await mydrivebase.gyrobackwards(100, 750, typestop = mydrivebase.braketank, initpid = 0); await wait(250)

        # ---> Leave all elements and get the shark (or what it is) <--- #
        await mydrivebase.gyroforwards(75, 225, typestop = mydrivebase.braketank, initpid = 1); await wait(250)
